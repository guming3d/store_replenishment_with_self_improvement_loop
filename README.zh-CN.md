...
每一行都附带生成它的 `(s, S)` 输入参数：需求预测、安全库存、提前期、现有库存与在途库存，推导算术清晰可见。

![补货建议](docs/screenshots/zh/suggestions.png)

### 2. 改数可以，但必须给理由

店长可以随时调整订货量。选择原因类型并填入说明后，系统会将这段文字**记录为待核验的主张，由引擎在后续环节验证**。
...
出现残差代表引擎现有假设仅能解释部分分歧。未被解释的增量将通过知识候选来寻找参数解。
...
- **`UNREACHABLE` 代表公式无解。** 例如店长将 48 改为 10，但整箱起订下限是 18——任何需求系数都无法推出 10，证明这次改数不属于需求判断。此时系统清空 `proposed_value` 并保留 `boundary_value`，阻止审核人员采纳已被引擎证明无效的数值，同时打上 `NO_CALIBRATABLE_CANDIDATE` 风险标记。
...
归因解决的是“为什么改”。单独靠审批只会沉淀店长偏好，结合 POS 实际销量回流才能验证改数效果：
...
- **缺货损失计入真实需求。** 货架卖空代表潜在需求未被满足，需补回缺失销量，避免小订货量被误判为最优。
...
- **无法生效的知识明确报错。** 比如 `SUBSTITUTION_RATE` 属于原始输入，无法作为引擎可调参数，系统会标记为 `unsupported`。
